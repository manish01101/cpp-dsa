...
private:
	...
	long count(const string& a, const string& b, int i, int limit, const string& s, bool tight1, bool tight2, vector<vector<vector<long>>>& mem) {
		if (i + s.length() == b.length()) {
			const string aMinSuffix = tight1 ? std::string(a.end() - s.length(), a.end()) : string(s.length(), '0');
			const string bMaxSuffix = tight2 ? std::string(b.end() - s.length(), b.end()) : string(s.length(), '9');
